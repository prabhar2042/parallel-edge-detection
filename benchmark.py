...
def run_sequential(image_name):
    ...
    ret = subprocess.run([f'./{bin_seq}', '-r', read_file, '-w',
                         f'{output}{write_file}'], capture_output=True, text=True)
    return ret.stdout
...
def run_openMP(image_name):
    ...
    #print (f'For openMP')
    ret = subprocess.run([f'./{bin_mp}', '-r', read_file, '-w',
                         f'{output}mp_{write_file}'], capture_output=True, text=True)
    return ret.stdout


def run_cuda(image_name):
    read_file = f'{dataset}{image_name}'
    write_file = f'out_{image_name}'
    bin_cuda = "parallel_CUDA/parallel_cuda"
    #print (f'For openMP')
    ret = subprocess.run([f'./{bin_cuda}', '-r', read_file, '-w',
                         f'{output}cuda_{write_file}'], capture_output=True, text=True)
    return ret.stdout
